...
Actionable notification events include new orders, settled or refunded payments, cancellation reviews, return requests, low-stock crossings, new customer reviews, support requests, scheduled reports, and protected system notices. Android separates order and payment alerts from the general operations channel and uses the monochrome CozyCraft furniture mark required by the Android status bar. iOS groups alerts by event type and displays the full-color CozyCraft Admin application icon supplied by the operating system. Notification taps retain the related entity and open the narrowest permitted destination in the app.
